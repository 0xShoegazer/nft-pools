...
contract xPAWG is Ownable, ReentrancyGuard, ERC20("PAWG Nation Escrowed Token", "xPAWG"), IXToken {
    ...
    struct RedeemInfo {
        uint256 protocolTokenAmount; // ProtocolToken amount to receive when vesting has ended
    ...
    IProtocolToken public immutable protocolToken; // ProtocolToken token to convert to/from
     ...
     * by transferring him "protocolToken" and removing "xTokenAmount" from supply
     *
    ...
    function _finalizeRedeem(address userAddress, uint256 xTokenAmount, uint256 protocolTokenAmount) internal {
        uint256 protocolTokenExcess = xTokenAmount.sub(protocolTokenAmount);
