...
contract YieldBooster is Ownable, ReentrancyGuard, IXTokenUsage, IYieldBooster {
    ...
    IXToken public immutable xToken; // XToken contract
    ...
    // User's position xToken total allocation
    ...
    mapping(address => uint256) private _usersTotalAllocation; // User's xToken total allocation
    ...
    function getMultiplier(
        ...
        return
            _getMultiplier(
                ...
                totalLpSupply,
                userAllocation,
                _poolsTotalAllocation[poolAddress]
            );
    }
    ...
    /**
     * Allocates "userAddress" user's "amount" of xToken to this usage contract
    ...
    function _getMultiplier(
        ...
        uint256 multiplier = userAllocation.mul(totalLpSupply).mul(maxBoostMultiplier).div(
            lpAmount.mul(totalAllocation_)
        );
    ...
    function _allocate(address userAddress, address poolAddress, uint256 tokenId, uint256 amount) internal {
        ...
        usersPositionsAllocation[userAddress][poolAddress][tokenId] = usersPositionsAllocation[userAddress][
            poolAddress
        ][tokenId].add(amount);
