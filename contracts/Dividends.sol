 ...
 * This distribution slot is the source of the dividends distribution to xToken allocators during the current cycle
 *
 * This transfer from the pending slot to the distribution slot is based on cycleDividendsPercent and CYCLE_PERIOD_SECONDS
 *
 */
contract Dividends is Ownable, ReentrancyGuard, IXTokenUsage, IDividends {
    ...
    address public immutable xToken; // xToken contract

    mapping(address => uint256) public usersAllocation; // User's xToken allocation
    uint256 public totalAllocation; // Contract's total xToken allocation
    ...
    /**
     * Allocates "userAddress" user's "amount" of xToken to this dividends contract
     *
     * Can only be called by xToken contract, which is trusted to verify amounts
     * "data" is only here for compatibility reasons (IXTokenUsage)
     */
    function allocate(
        address userAddress,
        uint256 amount,
    ...
    /**
     * Deallocates "userAddress" user's "amount" of xToken allocation from this dividends contract
     *
     * Can only be called by xToken contract, which is trusted to verify amounts
     * "data" is only here for compatibility reasons (IXTokenUsage)
     */
    ...
    function _updateDividendsInfo(address token) internal {
        ...
        // if no xToken is allocated or initial distribution has not started yet
    ...
    function _harvestDividends(address token) internal {
        ...
        uint256 userXTokenAllocation = usersAllocation[msg.sender];
        uint256 pending = user.pendingDividends.add(
            userXTokenAllocation.mul(accDividendsPerShare).div(1e18).sub(user.rewardDebt)
        );

        user.pendingDividends = 0;
        user.rewardDebt = userXTokenAllocation.mul(accDividendsPerShare).div(1e18);
