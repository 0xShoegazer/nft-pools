...
contract NFTPool is ReentrancyGuard, INFTPool, ERC721("Staking position NFT", "spNFT") {
    ...
    // Release all locks in case of emergency
    bool public emergencyUnlock;

    bool public initialized;

    // Used to delegate multiplier settings to project's owners
    address public operator;

    // Address of the MasterChef
    IMasterChef public master;

    // NFTPoolFactory contract's address
    address public immutable factory;

    // Deposit token contract's address
    IERC20Metadata private _lpToken;

    // ProtocolToken contract's address
    IERC20Metadata private _protocolToken;

    // XToken contract's address
    IXToken private _xToken;

    // Sum of deposit tokens on this pool
    uint256 private _lpSupply;

    // Sum of deposit token on this pool including the user's total multiplier (lockMultiplier + boostPoints)
    uint256 private _lpSupplyWithMultiplier;

    // Accumulated Rewards (staked token) per share, times 1e18. See below
    uint256 private _accRewardsPerShare;

    // 250%, high limit for maxGlobalMultiplier (100 = 1%)
    // All mulitpliers readable via getMultiplierSettings
    uint256 public constant MAX_GLOBAL_MULTIPLIER_LIMIT = 25000;
    // 150%, high limit for maxLockMultiplier (100 = 1%)
    uint256 public constant MAX_LOCK_MULTIPLIER_LIMIT = 15000;
    // 150%, high limit for maxBoostMultiplier (100 = 1%)
    uint256 public constant MAX_BOOST_MULTIPLIER_LIMIT = 15000;

    uint256 private _maxGlobalMultiplier = 20000; // 200%

    // 6 months, Capped lock duration to have the maximum bonus lockMultiplier
    uint256 private _maxLockDuration = 183 days;

    // 100%, Max available lockMultiplier (100 = 1%)
    uint256 private _maxLockMultiplier = 10000;

    // 100%, Max boost that can be earned from xToken yieldBooster
    uint256 private _maxBoostMultiplier = 10000;

    // 100%, high limit for xTokenRewardsShare
    uint256 private constant _TOTAL_REWARDS_SHARES = 10000;
    // 80%, directly defines protocolTokenShare with the remaining value to 100%
